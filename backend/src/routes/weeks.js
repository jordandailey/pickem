...
// PUT /api/weeks/:id - admin: edit week details
router.put('/:id', authMiddleware, adminMiddleware, async (req, res) => {
  const { nfl_week, cfb_week, label, submission_deadline, quarter_id } = req.body;
  try {
    const { rows } = await pool.query(`
      UPDATE weeks SET nfl_week=$1, cfb_week=$2, label=$3, submission_deadline=$4::timestamptz, quarter_id=$5::uuid
      WHERE id=$6 RETURNING *
    `, [nfl_week, cfb_week, label || null, submission_deadline, quarter_id || null, req.params.id]);
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
